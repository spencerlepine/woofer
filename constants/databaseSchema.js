...
const schema = (Joi) => {
  ...
  outputObj[DATA_KEYS["CHAT_USERS"]] = Joi.array().items(outputObj[DATA_KEYS["USER_ID"]]);
  ...
  outputObj[DATA_KEYS["USER_CHATS"]] = (
    Joi.array().items(outputObj[DATA_KEYS["CHAT_INFO"]])
  );
  ...
  outputObj[DATA_KEYS["USER_PROFILE"]] = (
    Joi.object()
      .keys(userProfileObj)
      .unknown()
  );
